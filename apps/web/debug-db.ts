...
async function debug() {
  try {
    ...
    events.forEach((e, i) => {
      console.log(
        `  ${i + 1}. ${e.eventName} on chain ${e.chainId} - Processed: ${
          e.processed
        }`
      );
      console.log(
    ...
    indexerStates.forEach((state) => {
      console.log(
        `  Chain ${state.chainId}: Last block ${state.lastBlockNumber}`
      );
    });
    ...
    orders.forEach((o, i) => {
      console.log(
        `  ${i + 1}. Order ID: ${o.orderId}, Chain: ${o.chainId}, Status: ${
          o.status
        }`
      );
    ...
    chainConfigs.forEach((cc) => {
      console.log(
        `  Chain ${cc.chainId} (${cc.name}): Last indexed block ${cc.lastIndexedBlock}`
      );
    });
  } catch (error) {
