  ...
  dsc: {
    ...
    usdtAddress: normalizeAddress(
      process.env.NEXT_PUBLIC_CHAIN_B_USDT_CONTRACT ||
        "0xbc27aCEac6865dE31a286Cd9057564393D5251CB"
    ),
  },
...
async function getPendingFillOrders(): Promise<PendingFillOrder[]> {
  try {
    const [orderIds, sellers, buyers, amounts, bscOrderIds] =
...
async function getOpenBuyOrders(): Promise<OpenBuyOrder[]> {
  try {
    const [orderIds, users, amounts, remainingAmounts, expiresAts] =
      await bscPublicClient.readContract({
...
async function executeFillOrder(
  pendingOrder: PendingFillOrder
): Promise<boolean> {
  ...
  try {
    ...
    if (alreadyUsed) {
      ...
      const dscTxHash = keccak256(
        encodePacked(
          ["uint256", "uint256", "address"],
          [dscOrderId, bscOrderId, seller]
        )
      );
      ...
      const bscReceipt = await bscPublicClient.waitForTransactionReceipt({
        hash: bscTxHash,
      });
    ...
    const bscProofHash = keccak256(
      encodePacked(
        ["uint256", "uint256", "address"],
        [bscOrderId, dscOrderId, buyer]
      )
    );
    ...
    const dscReceipt = await dscPublicClient.waitForTransactionReceipt({
      hash: dscTxHash,
      timeout: 60000,
    });
...
async function matchAndExecuteOrders(
  ...
  for (const buyOrder of buyOrders) {
    for (const sellOrder of sellOrders) {
      ...
      console.log(
        `   BSC Buyer: ${buyOrder.user} wants ${formatUnits(
          buyOrder.remainingAmount,
          18
        )} DEP20`
      );
      console.log(
        `   DSC Seller: ${sellOrder.user} selling ${formatUnits(
          sellOrder.remainingAmount,
          18
        )} DEP20`
      );
      ...
      try {
        ...
        console.log(
          `   🎉 SWAP COMPLETE! ${formatUnits(matchAmount, 18)} USDT swapped`
        );
...
async function scanAndExecute() {
  ...
  for (const order of openBuyOrders) {
    console.log(
      `     #${order.orderId}: ${formatUnits(
        order.remainingAmount,
        18
      )}/${formatUnits(order.amount, 18)} USDT by ${order.user.slice(0, 8)}...`
    );
  }
  ...
  for (const order of pendingFills) {
    console.log(
      `     #${order.dscOrderId} → BSC #${order.bscOrderId}: ${formatUnits(
        order.amount,
        18
      )} USDT`
    );
...
async function main() {
  ...
  console.log(
    "           P2P Exchange Relayer V2 - Auto Execution            "
  );
  console.log(
    "═══════════════════════════════════════════════════════════════"
  );
  ...
  // Check balances
  const bscBalance = await bscPublicClient.getBalance({
    address: account.address,
  });
  const dscBalance = await dscPublicClient.getBalance({
    address: account.address,
  });
